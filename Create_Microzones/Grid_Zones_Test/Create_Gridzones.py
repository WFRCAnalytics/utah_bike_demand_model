...
maz_clipped = arcpy.Clip_analysis(grid_zones, taz_layer, microzones_geom)
...
arcpy.CalculateField_management(maz_clipped,"zone_id",'!{}!'.format('FID'))
...
arcpy.SpatialJoin_analysis(maz_clipped, taz_polygons, microzones,'JOIN_ONE_TO_ONE', '', '', 'HAVE_THEIR_CENTER_IN')
...
parcels = parcels[['parcel_id', 'SHAPE']].copy()
...
maz_remm_data = maz_remm_data[['zone_id', 'CO_TAZID', 'TAZID', 'CO_FIPS', 'CO_NAME', 'residentia', 'households', 'population', 'jobs1', 'jobs3', 'jobs4', 'jobs5', 'jobs6', 'jobs7', 'jobs9', 'jobs10', 'SHAPE']].copy()
...
maz_centroid_join_df =  maz_centroid_join_df[['zone_id', 'NEAR_FID']].copy()
...
maz_remm_data2 = maz_remm_data
for field in list(maz_remm_data2.columns):
    if field not in ['SHAPE']:
        maz_remm_data2[field].fillna(0, inplace=True)

# final export
final_zones = os.path.join(temp_dir, "microzones.shp")
maz_remm_data2.spatial.to_featureclass(location=final_zones)
